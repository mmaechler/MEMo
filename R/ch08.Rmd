...
- taking advantage of _partially linear_ models, so that initla estimates are needed only for those parameters that enter the model **non**linearly; in R, read docu on  `nls(*, method="plinear")`.
...
## 8.2 Fitting Nonlinear Mixed-Effects Models with nlme  (p. 345)
...
Now the Theophylline modelling via NLME :

```{r, nlme-Theo, warning = FALSE}
(fm1Theo.nlme <- nlme(fm1Theo.lis))
intervals(fm1Theo.nlme, which="var-cov") ## (now work)
