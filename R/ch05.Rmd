...
options(width = 75  # MM: width 65 is too narrow
      ...
      , contrasts = c(unordered = "contr.helmert",
                        ordered = "contr.poly") )
if(!dev.interactive(TRUE)) pdf(file = 'ch05_plot.pdf')
...
### 5.3.4  Using Correlation Structures with `lme()`,  p.239
...
anova(fm2Ovar.lme, fm3Ovar.lme, test = FALSE)# fm2 is better
fm4Ovar.lme <- update(fm1Ovar.lme,
                       correlation = corCAR1(form = ~Time))
anova(fm2Ovar.lme, fm4Ovar.lme, test = FALSE) # still slightly better
...
So, `fm5...` is best, and looking at the final residual auto correlation (Figure 5.12, p.245) we conclude they look like uncorrelated:
```{r, save-fmOvar, echo=FALSE}
saveRDS(fm5Ovar.lme, file = "fm5Ovar_lme.rds") # -> needed in ./ch08.Rmd
```{r plot-ACF-fm5.Ovar}
plot(ACF(fm5Ovar.lme,  maxLag = 10, resType = "n"), alpha = 0.01)
```

#### Body Weight Growth in Rats (p. 244)

Now using `"corStruct"` classes in addition to previous model:
```{r, Variogram_BW}
...
## 5.4  Fitting Extended Linear Models with `gls()`,  p.249
