...
```{r, setup }
library(nlme)
if(!dev.interactive(TRUE)) pdf(file = 'ch01.pdf')
...
with $i = 1,2,\dots,m (= 6)$, for $j=1,\dots,n_i ( = 6)$, for $n$ observations $n = \sum_{i=1}^m$,
and the usual assumption
$$\varepsilon_{i,j} \mathrm{i.i.d.} \ \sim  \mathcal{N}(0, \sigma^2).$$


```{r, Rail-lm}
## if we have an intercept, cannot use all 6 levels (singular matrix):
fm2.Rail.lm <- lm( travel ~ Rail, data = Rail.u )
summary(fm2.Rail.lm) # R's default is 'treatment' contrast: 1st level := 0
## In this case of  1-way ANOVA: Nicer to drop the intercept
fm2Rail.lm <- lm( travel ~ Rail - 1, data = Rail.u )
summary(fm2Rail.lm)
anova(fm2Rail.lm)
```
and the Rail effect _is_ significant.  Alternatively, as the rails are
